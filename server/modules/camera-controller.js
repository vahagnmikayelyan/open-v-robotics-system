...
class CameraController extends EventEmitter {
    constructor() {
        ...
        this.photoWidth = 1280;
        this.photoHeight = 720;
    ...
    startVideo() {
        ...
        this.videoProcess = spawn('rpicam-vid', [
            '--codec', 'mjpeg',
            '--width', this.videoWidth.toString(),
            '--height', this.videoHeight.toString(),
            '--framerate', this.videoFramerate.toString(),
            '--autofocus-mode', 'continuous', // Need for Arducam 3
            '--nopreview',
            '--timeout', '0',
            '--output', '-'
        ]);

        this.videoProcess.stdout.on('data', (chunk) => this.handleData(chunk));

        this.videoProcess.stderr.on('data', () => {}); // Ignoring libcamera logs
    ...
    async takePhoto(width = 0, height = 0) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const photoProcess = spawn('rpicam-still', [
                '--width', (width || this.photoWidth).toString(),
                '--height', (height || this.photoHeight).toString(),
